...
- `kernel/` — the kernel crate (`jos`).
  ...
  - `kernel/src/elf.rs` — minimal ELF64 loader for an embedded user image.
  - `kernel/src/process.rs` — per-process state (address space, loaded image, frames, ring); the process table.
  - `kernel/src/programs.rs` — registry of embedded programs (name → ELF), generated by `build.rs`.
  - `kernel/src/user.rs` — drop to EL0 in the current process; pointer validation, `copy_to_user`.
  - `kernel/build.rs` — builds the `user` crate and embeds every `src/bin/` program, generating the registry.
...
- `user/` — the EL0 userspace crate (separately compiled): a runtime lib plus one binary per program.
  - `user/src/lib.rs` — the shared runtime ("libu"): print/line helpers, `exit`, the panic handler.
  - `user/src/uring.rs` — userspace half of `jring`: setup/sqe/submit/wait.
  - `user/src/bin/jsh.rs` — `jsh`: prompt, line editing, `help`/`spam`/`crash`/`exit` builtins.
  - `user/src/bin/echo.rs` — `echo`: prints a line and exits (a second, distinct program).
  - `user/user.ld` — EL0 VA layout (shared by every program) with separate R-X / R-W segments.
